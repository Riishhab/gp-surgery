...
try {
    ...
    // Prepare the subquery for new entries from the global database
    $subquery = "SELECT NHSNumber, DoseNo FROM Vaccines WHERE NHSNumber IS NOT NULL";
    $subqueryStmt = $localPDO->query($subquery);
    $existingKeys = $subqueryStmt->fetchAll(PDO::FETCH_ASSOC);

    // Retrieve new entries from the global database
    $globalQuery = "SELECT * FROM vaccines WHERE (NHSNumber, DoseNo) NOT IN (SELECT NHSNumber, DoseNo FROM (" . $subquery . "))";
    $globalStmt = $globalPDO->query($globalQuery);
    $data = $globalStmt->fetchAll(PDO::FETCH_ASSOC);
    var_dump($data);
    ...
    foreach ($data as $row) {
        $key = array('NHSNumber' => $row['NHSNumber'], 'DoseNo' => $row['DoseNo']);
        if (!in_array($key, $localKeys)) {
            $insertStmt->bindValue(':NHSNumber', $row['NHSNumber']);
            $insertStmt->bindValue(':DoseNo', $row['DoseNo']);
            $insertStmt->bindValue(':vaccinationDate', $row['VaccinationDate']);
            $insertStmt->bindValue(':vaccineManufacture', $row['VaccineManufacture']);
            $insertStmt->bindValue(':diseaseTargeted', $row['DiseaseTargeted']);
            $insertStmt->bindValue(':vaccineType', $row['VaccineType']);
            $insertStmt->bindValue(':product', $row['Product']);
            $insertStmt->bindValue(':vaccineBatchNumber', $row['VaccineBatchNumber']);
